...
use crate::{CollectSink, FileResult, Hit, HitKind, Mode, Options, Rung, ScanResult, Source, Stats, build_matcher, clip_line};
use anyhow::Result;
...
pub fn scan(o: &Options, data: Vec<u8>) -> Result<ScanResult> {
    ...
    for lh in sink.hits {
        ...
        let (ms, me) = lh.subs[0];
        let le = memchr::memchr(b'\n', &bytes[ls as usize..]).map(|k| ls as usize + k).unwrap_or(bytes.len());
        let line_bytes = &bytes[ls as usize..le];
        let me_line = (me as usize).min(le) as u32;
        let (text, clipped, tm) = clip_line(line_bytes, (ms - ls) as usize, (me_line - ls) as usize, o.max_columns);
        let raw = line_bytes.strip_suffix(b"\r").unwrap_or(line_bytes).to_vec();
        hits.push(Hit { line: lh.line, line_start: ls, match_start: ms, match_end: me, submatches: lh.subs, kind: HitKind::Ident, chain: Vec::new(), def_idx: None, score: 1.0, text, text_match: (tm.0 as u32, tm.1 as u32), clipped, raw });
    }
    ...
    if total > 0 {
        files.push(FileResult { rel: STDIN_NAME.into(), path: STDIN_NAME.into(), lang: Lang::None, flags: FileFlags::default(), size: bytes.len() as u64, age_days: 0.0, mtime: 0, prior: 1.0, hits, total, total_unfiltered: total, kinds, defs: Vec::new(), refined: true, file_id: None, src: Some(src) });
    }
    let mut stats = Stats { files_walked: 1, files_searched: 1, files_matched: files.len(), total_hits: total, total_unfiltered: total, threads: 1, source: "stdin", ..Default::default() };
...
mod tests {
    ...
    #[test]
    fn stdin_scan_lines() {
        let o = Options { pattern: "a".into(), budget: 0, ..Default::default() };
        let r = scan(&o, b"a\nb\nxa\n".to_vec()).unwrap();
