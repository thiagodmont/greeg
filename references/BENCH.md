...
Merged main against the JSON exit status, detached index directory, generated-file, ignore-input digest and regular-file fixes. `--fresh stat` runs the freshness check, including the ignore-input digest, on every query. All 30 cases keep identical stdout, exit status and tokens, and every ripgrep comparison passes. Scan medians change by +0.4% and index medians by +1.8% (median across cases). One p95 flag, scan case_miss_files +36.4% with a +0.5% median, is on a path that never computes the digest. Host load average was 10.8–11.5 from an unrelated workload. The harness records no execution time; the result file was written at 2026-09-23T19:02:41Z.
