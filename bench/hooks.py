...
Requires rg. Token counts describe hook protocol replies, not agent-task savings.
"""
import argparse
import hashlib
import json
...
def main():
    ...
    report = {"protocol": 1, "platform": platform.platform(), "machine": platform.machine(),
    ...
    with tempfile.TemporaryDirectory(prefix="greeg-hooks-") as tmp:
        ...
        for agent in ("claude", "codex"):
            for case, (command, accepted) in CASES.items():
                ...
                for label in binaries:
                    ...
                    exact = rewritten is not None and shlex.split(rewritten)[1:3] == ["--matching", "exact"]
        ...
        for label, binary in binaries.items():
            ...
            for backend in ("scan", "index"):
                for command in ("rg -l needle", "rg -c needle", "rg -l NEEDLE", "rg -w -c needl",
                                "rg -i -l NEEDLE", "rg -F -l 'needle()'"):
                    payload = json.dumps({"tool_name": "Bash", "tool_input": {"command": command}}).encode()
