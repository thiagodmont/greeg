...
def fixtures(agent):
    ...
    def config(handlers, matcher="Bash", label=False):
        entry = {"matcher": matcher, "hooks": handlers}
        if label:
            entry["label"] = "shared"
    ...
    def encode(value):
        ...
        entry = entries[0]
        lines = ["# fixture", "[[hooks.PreToolUse]]", f"matcher = {json.dumps(entry['matcher'])}"]
        if "label" in entry:
    ...
    return [
        ("mixed_uninstall", True, encode(mixed), config([other], label=True), 0),
        ("prefix_uninstall", True, encode(prefix), encode(prefix), 0),
        ("prompt_uninstall", True, encode(prompt), encode(prompt), 0),
        ("wrong_matcher_install", False, encode(wrong), appended, 0),
        ("invalid_utf8", False, b"{\xff}", b"{\xff}", 2),
        ("absent_uninstall", True, None, None, 0),
        ("empty_install", False, None, installed, 0),
    ]
...
def invoke(binary, agent, case, base, env):
    ...
    _, uninstall, original, expected, code = case
    ...
    if isinstance(expected, dict):
        try:
            ...
            actual = None
    return {"ms": elapsed, "contract": run.returncode == code and actual == expected,
...
def main():
    ...
    report = {"protocol": 1, "platform": platform.platform(), "python": platform.python_version(),
        ...
        for agent in ("claude", "codex"):
            for case in fixtures(agent):
                ...
                for metric in ("median", "p95"):
                    row[f"{metric}_change_percent"] = (row["candidate"][f"{metric}_ms"] / row["baseline"][f"{metric}_ms"] - 1) * 100
