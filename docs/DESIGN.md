...
Status: v1.3, 2026-09-01, updated through M6 (implementation notes are marked "As shipped"). Companion to `PLAN.md`. Every number in this
...
| Flag | Rule |
|---|---|
| `BINARY` | NUL byte in the first 8 KiB, or > 30 % bytes outside printable ASCII/UTF-8 |
| `MINIFIED` | `max_line_len > 1000` and `avg_line_len > 200`, or a `.min.` in the name, or a `//# sourceMappingURL` footer with `avg_line_len > 120` |
| `GENERATED` | header within first 2 KiB matching `@generated`, `DO NOT EDIT`, `Code generated by`, `autogenerated`, `This file was automatically generated`; or path segment `generated`, `__generated__`, `gen`, `.pb.`, `_pb2.py`, `.g.dart`, `*.designer.cs`; or `packages/*/src/compiled` |
| `VENDORED` | path segment `vendor`, `vendored`, `third_party`, `thirdparty`, `node_modules`, `external`, `deps`, `.yarn`, `bower_components` |
| `LOCKFILE` | `package-lock.json`, `yarn.lock`, `pnpm-lock.yaml`, `Cargo.lock`, `poetry.lock`, `uv.lock`, `Pipfile.lock`, `gradle.lockfile`, `pixi.lock` |
| `TEST` | path segment `test`, `tests`, `__tests__`, `spec`, `testing`, `testdata`, `fixtures`, `snapshots`; or name matching `*_test.*`, `test_*.py`, `*.test.*`, `*.spec.*`, `*Test.kt`, `*Tests.kt` |
| `HUGE` | size > 4 MiB |
