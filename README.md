...
```
v=0.4.0; t=aarch64-apple-darwin   # or x86_64-apple-darwin, x86_64-unknown-linux-gnu, aarch64-unknown-linux-gnu
...
its wall time and output size. `greeg stats replay` runs the original
`rg`/`grep` commands and their greeg rewrites side by side, on the same
machine and the same tree, and `greeg stats` then leads with the savings:

```
greeg stats enable
# ... work with the agent for a while ...
greeg stats replay --runs 3          # on a quiet machine
greeg stats --since 7d --repo .      # --json for machine output, --verbose to list every query
```
...
greeg show tokio/src/sync/oneshot.rs:340   # the definition enclosing a line, whole
greeg def JoinHandle --mode block           # every definition of the name with its body
